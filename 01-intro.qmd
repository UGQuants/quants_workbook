...
- `R Markdown`: a file format that mixes narrative text and code; when `knit`, the results from the code are woven into the 'knitted' document.
- `code chunk`: a fenced block in an R Markdown file for adding R code; as well as running when you knit the file, each code chunk can be run on its own within RStudio.
- `knit`: render an R Markdown file into a specified format (e.g. HTML or PDF) by running all code chunks from top to bottom and creating a knitted document combining the text with the code results.
...
::: {.callout-tip title="New Terms"}
- `object`: any data or function stored in memory and usually bound to a name in an environment.
- `assignment`: bind a name to an object in an environment with `<-`, creating or updating that binding (e.g. `a <- 10`).
- `environment`: a workspace that maps names and objects (the main one is the Global Environment, which can see in the top-right panel in RStudio).
:::

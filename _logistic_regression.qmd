...
As we discussed above, we are interested in modelling the probability of an event occurring. However, probabilities are non-linear. This makes it a bit tricky to interpret their relationship with the independent variables. Fortunately, the link function helps us to establish a linear relationship between the dependent $p$ and the independent variable $\beta$. The logit model can be expressed as follows:
...
As a generic interpretation, we can say that the estimated coefficients increase or decrease the chance of the outcome happening.
...
By exponentiating the coefficients, our results will not change; they will only be expressed on a different scale.

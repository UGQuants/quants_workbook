...
Note, creating a reduced version uses another **selection operator** available with the tidyverse's improved version of base R's data frames. The basic format is `data_frame[rows, columns]`. Here `starwars[, 1:11]` leaves the `rows` argument blank, which effectively means select all rows, and the `columns` argument has `1:11`, which effectively means select columns 1 to 11. So, the code returns a copy of the data frame with all rows but only the first 11 columns.
...
The `sum()` function adds the value of two numbers in format `sum(x, y)`. Using a pipe for `1 |> sum(1)` is the equivalent to `sum(1, 1)`. If we were to write `sum(x, y)` with a pipe, it would be `x |> sum(y)`. So, in our exaple code chunk, the pipe let's us 'unpack' the first argument in each function, whereby line 1 reads equivalent to saying 'take 1, then add it with 1' and line 2 'take 1, then add it with 1, then add it with 5'.

This may seem as if it is overcomplicating things, but as the code becomes more complex the value of the pipe operator becomes clearer. The second line, `1 |> sum(1) |> sum(5)` would be written as `sum(sum(1,1), 5)` without pipes. Going left to right, `1 |> sum(1)` would be `sum(1,1)` and then `sum(1,1) |> sum(5)` would be `sum(sum(1,1), 5)`. Pipes then help us avoid `foo(foo(foo(...), ...) ...)` monstrosities in our code. (In coding examples, you will see often see words like `foo`, `bar` and `baz` used as placeholder names when explaining how coding syntax works rather than using real functions and other objects.)

As we will cover in examples in the next section below, plenty of functions take more than two arguments. Let's imagine we had a data frame `df` and we were updating it by using - in order - `foo`, `bar`, and `baz` functions that each take arguments x, y, and z. Without pipes our code would be `df <- baz(bar(foo(df, y, z), y, z) y, z)`. This is a tangled mess, we want the result from `foo` passed to `bar` and its result in turn passed to `baz`. This ordering though gets visually reversed when nesting each function within the other - we have to read it inside out for it to make sense.
...
Now it is easy to read and the ordering of the functions matches how we could verbally describe what the code does, "we are updating a data frame called `df` by running the functions `foo`, `bar` and `baz` on it in turn". In the code chunk above, we make use of another wonderful thing about pipes - it let's us break complex code over multiple lines. To do so you add the pipe at the end of the line after a function, then start the new line with an indent, usually 2 spaces by default.
...
Before coercing these variables, we will create a smaller subset selecting only the variables that we are interested in. To do so, we can use the `select()` function. The `select()` function takes the name of the data first and then the name of the variables we want to keep (no quotation marks needed). Remember with pipes we can replace `select(data_frame, variable1, variable2, ...)` with `data_frame |> select(variable1, variable2, ...` making it easer to read as "from our data frame select the following variables". Let's create a code chunk to select a few variables and assign the result to a new object called `police_2`.

```{r}
police_2 <- police |> select(age, gender, raceethnicity, lawenforcementagency, armed)
```

Note, `select()` is another function from the `dplyr` tidyverse package. Many tidyverse functions that take a data frame as the first argument, `foo(first_argument, ...)` or with pipe `first_argument |> foo(...)`, will assume all variables listed after are from that data frame. Within the function, this then let's us type `age` and `gender` rather than `police$age` and `police$gender`. Notice how with pipes this results in everything listed within the function just being the variables we are interested in.
...
As a final side note - and a wee preview of code you will see in later labs and in R textbooks - all the steps we did above for creating and modifying `police_2` can also be written in a single code chunk. In this version, we do all the same steps, we create `police_2` by first selecting variables from `police`, then coercing the types of some variables, and finally filtering the data.
...
Notice here that we can also pass multiple variables to `mutate()` by separating each with a commma. R also lets us split anything after a bracket or comma onto a new line. With long functions, it is common to start a new line after the `(`, then add each element that is separated by a comma on a line of its own, then add a final line with the closing `)`. Notice as well that the elements are indented as further two spaces and the closing `)` is indented the same as `mutate(`.

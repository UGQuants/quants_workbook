...
`title` and `author` are self-explanatory, but make sure to replace `[your name here]` with your name.

`date` can be any text, but `format(Sys.time(), '%d/%m/%y')` gets the current date and formats it as dd/mm/yyyy, such as 12/09/2025.
...
![r setup code chunk with knitr options to set message and false to FALSE](./images/rstudio_lab2_setupchunk.png)
...
What you will also notice is the message raised in the Console why loading the tidyverse. This is one of the reasons why we have the have the setup chunk to prevent messages and warnings from being included in knitted files. If we were to knit Lab-02.Rmd file without the setup chunk, the message on loading the tidyverse would be added into the knitted HTML file after the preamble chunk.

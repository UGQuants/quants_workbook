...
We will continue working on the same RStudio Cloud project as in the previous session and using the 2012 [Northern Ireland Life and Times Survey (NILT)](https://www.ark.ac.uk/nilt/) data. To set the `R` environment please follow the next steps:
...
3. Create a new Rmd file, type 'Correlation analysis' in the 'Title' section and your name in the 'Author' box. Leave the 'Default Output Format' as `HTML`.

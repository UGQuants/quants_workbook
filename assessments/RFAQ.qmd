...
Indeed, all genAI responses are [bullshit](https://link.springer.com/article/10.1007/s10676-024-09775-5) - and I use that as the correct philosophical term to describe it. GenAI does not distinguish between fact and fiction, it merely selects the next most probable token - which for simplicity can think of as a word - based on the tokens fed to it.
...
![Copilot bullshit response](../images/sumtable_wrong.png)

The `sumtable()` function, as we covered in [Lab 4](../04-Lab4.qmd), doesn't come from the `stargazer`, `modelsummary`, nor `summarizer` packages. Instead, it comes from `vtable`. Yet, after telling Copilot that I still receive an error after installing the packages it advised:

![Copilot continuing to bullshit](../images/copilot_fail.png)
...
Indeed, if you review [Lab 4](../04-Lab4.qmd), you will find that the solution to the problem was easy - install and load the package we used in the lab that provided the `sumtable()` function -
...
As a reminder, whilst I have setup a custom GPT / saved prompt - [RStudio Helper GPT](../help/rstudio_helper.qmd) - that aims to mitigate the worst aspects of standard genAI responses on R, it remains genAI with all the flaws that entails. You should still treat all its responses as what they are - bullshit.
